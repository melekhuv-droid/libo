-- =============================================================================
--  mhlibrary.lua - Melek Hub UI library (Obsidian-based)
--  Integrated from the KiciaHook open-source RIVALS build.
--  Usage:
--      local Library = loadstring(game:HttpGet(".../mhlibrary.lua"))()
--      local Window  = Library:CreateWindow({
--          Title  = "Melek Hub",
--          Footer = "<font color=\"#...\">your footer text</font>",
--      })
--  Returns the Library table (Obsidian-compatible API).
-- =============================================================================

-- Executor global-environment fallback: keeps bare getgenv() calls inside the
-- library safe on executors whose getgenv is broken or missing.
local getgenv = getgenv or function()
    return shared
end

-- Shared module registry (same contract as the KiciaHook build's
-- RequireSharedModule): modules are registered below and required lazily,
-- cached after the first call.
local __mh_shared = {}

local __mh_shared_cache = {}

local function RequireSharedModule(name)
    if type(__mh_shared) ~= "table" then
        error("Missing shared module table")
    end

    local cached = __mh_shared_cache[name]
    if cached ~= nil then
        return cached
    end

    local loader = __mh_shared[name]
    if type(loader) ~= "function" then
        error("Missing shared module: " .. tostring(name))
    end

    local result = loader()
    __mh_shared_cache[name] = result
    return result
end

__mh_shared.obsidian_icons = function()
...
__mh_shared.obsidian_library = function()
...
function Library:CreateWindow(WindowInfo)
    ...
    local InitialLeftWidth = math.min(math.ceil(WindowInfo.Size.X.Offset * 0.3), 100)
    ...
    do
        ...
        if WindowInfo.Icon then
            ...
            WindowIcon = New("ImageLabel", {
                AnchorPoint = Vector2.new(1, 0.5),
                Image = "rbxassetid://126781566929791",
                Position = UDim2.new(1, -6, 0.5, 0),
                Size = WindowInfo.IconSize,
                Visible = true,
                Parent = TitleHolder,
        ...
        WindowTitle = New("TextLabel", {
            ...
            Size = UDim2.new(1, -(WindowInfo.IconSize.X.Offset + 16), 0, 24),
    ...
    function Window:SetSidebarWidth(Width)
        Width = math.clamp(Width, 48, math.min(MainFrame.Size.X.Offset - WindowInfo.MinContainerWidth - 1, 100))

        DividerLine.Position = UDim2.fromOffset(Width, 0)

        TitleHolder.Size = UDim2.new(0, Width, 1, 0)
        CurrentTabInfo.Position = UDim2.new(0, Width + 10, 0.5, 0)
    ...
    function Window:AddTab(...)
        ...
        do
            ...
            TabLabel = New("TextLabel", {
                ...
                TextTransparency = 0.5,
                TextXAlignment = Enum.TextXAlignment.Left,
                Visible = not Icon and not IsCompact,
                Parent = TabButton,
            })
...
--  Return the library
-- =============================================================================
return RequireSharedModule('obsidian_library')
